...
// is retried until acknowledged." The main unlock path (verify.ts, called
// right after the redirect back) doesn't strictly need it -- it re-asks
// Dodo's API directly. This route is what actually emails the license key --
...
// (with an occasional deliberate duplicate against verify.ts, see license.ts).
...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (event.type === "payment.succeeded") {
    try {
      ...
      // are confirmed fields on that type.
